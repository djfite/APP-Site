...
function calc() {
  ...
  set('rCur', fmt2(curFees));
  set('rBluu', fmt2(bluuFees));
  set('rSave', fmt(savings));
  set('rAnnual', fmt(annual));
  ...
  const nudge = $('cNudge');
  if (nudge) {
    nudge.style.display = savings > 50 ? 'block' : 'none';
    const na = $('cNudgeAmt'); if (na) na.textContent = fmt(annual);
  }
}
